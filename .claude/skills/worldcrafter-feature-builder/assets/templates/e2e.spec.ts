...
test.describe('Feature E2E Tests', () => {
  ...
  test('submits form successfully', async ({ page }) => {
    // Fill form fields
    await page.fill('input[name="title"]', 'Test Feature Title')
    ...
    // Verify success (adjust based on your success indicator)
  ...
  test('displays validation errors for invalid input', async ({ page }) => {
    ...
    await expect(page.locator('text=/title.*too short/i')).toBeVisible()
  })
  ...
  test('handles server errors gracefully', async ({ page }) => {
    // Mock a server error (if using MSW or similar)
    // Or test with invalid data that triggers a server error
